...
void DrawSceneWithProjection()
{
	...
	// Compute light with Phong lighting.
	...
	// Compute projection color.
	vec3 ambientColor = LightAmbient * MatlAmbient;
	vec3 diffuseColor = Color.rgb * MatlDiffuse * N_dot_L;
	vec3 specularColor = Color.rgb * MatlSpecular * R_dot_V_pow_n;
	...
	// Use both the diffuse and specular coefficient of the light source 
	// for lighting the fragment.
    FragColor = vec4(shadowFact * (diffuseColor + specularColor) + ambientColor, 1.0 );
}
